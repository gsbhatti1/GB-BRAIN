...
function main(){
    $.CTA("BTC_USDT", function(st){
        ...
        if(mp == 1 && (close < midPrice)) return -1; //If you hold a long order and the closing price is less than the mid-rail, close position
        if (mp == -1 && (close > midPrice)) return 1; //If you hold a short order and the closing price is greater than the mid-rail, close position
        if (mp == 0 && close > midPrice) return 1; //If there is no position, and the closing price is greater than the upper track, open long position
        if (mp == 0 && close < downPrice) return -1; //If there is no position, and the closing price is less than the lower track, open short position
    });
}
```
