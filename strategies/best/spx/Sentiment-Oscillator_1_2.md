...
// Strategy Logic
if longStrategy and (entryCond == "Crossing Over" and entryVar1_ > 0.0 and entryVar2_ > 0.0) or (entryCond == "Crossing Under" and entryVar1_ < 0.0 and entryVar2_ < 0.0)
    strategy.entry("Long", strategy.long)

if longStrategy
    if exitCond == "Crossing Over" and macd > signal
        strategy.exit("Exit Long", "Long")
    else if exitCond == "Crossing Under" and macd < signal
        strategy.exit("Exit Long", "Long")

plot(showMs ? marketSentiment : na, title="Market Sentiment Line", color=color.blue, linewidth=2)
plot(showHist ? hist : na, title="Momentum Histogram", color=color.red, histogram=histogram.barcolor, linewidth=2)
plot(showMacd ? macd : na, title="MACD Line", color=color.green, linewidth=2)
plot(showSignal ? signal : na, title="Signal Line", color=color.orange, linewidth=2)
