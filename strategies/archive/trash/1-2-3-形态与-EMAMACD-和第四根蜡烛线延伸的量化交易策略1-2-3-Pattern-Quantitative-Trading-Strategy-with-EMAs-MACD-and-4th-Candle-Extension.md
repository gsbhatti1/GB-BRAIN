...
1-2-3 Pattern and EMAs-MACD with Fourth Candle Extension Quantitative Trading Strategy
...
This Pine Script-based strategy aims to identify potential buy and sell signals by combining the 1-2-3 pattern, additional conditions involving Exponential Moving Averages (EMAs) and the Moving Average Convergence Divergence (MACD) indicator. The strategy leverages price patterns, trend confirmation, and momentum indicators to provide comprehensive trading signals.
...
buy_candle3_above_candle1 = close[1] > close[3]
buy_candle4_above_candle3 = close[0] > close[3]
buy_condition = buy_candle1_above_open and buy_candle2_below_open and buy_candle3_above_candle1 and buy_candle4_above_candle3
...
sell_candle3_below_candle1 = close[1] < close[3]
sell_candle4_below_candle3 = close[0] < close[3]
sell_condition = sell_candle1_below_open and sell_candle2_above_open and sell_candle3_below_candle1 and sell_candle4_below_candle3

// Define EMA and MACD conditions
ema9 = ta.ema(close, 9)
ema20 = ta.ema(close, 20)
macd_line = ta.macd(close, 12, 26, 9)[0]
macd_signal = ta.macd(close, 12, 26, 9)[1]
macd_hist = ta.macd(close, 12, 26, 9)[2]

// Entry rules
long_entry = buy_condition and close > ema9 and close > ema20 and macd_line > macd_signal
short_entry = sell_condition and close < ema9 and close < ema20 and macd_line < macd_signal

// Exit rules
long_exit = not buy_condition or close[1] < close[0]
short_exit = not sell_condition or close[1] > close[0]
