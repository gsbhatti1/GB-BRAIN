...
class tbtest(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['sma_offset'] = ta.SMA(dataframe, timeperiod=30) * 0.970

        return dataframe

    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(
                              (dataframe['close'] < dataframe['sma_offset']) & (dataframe['rsi'] < 30)
                      ), ['buy', 'buy_tag']] = (1, 'buy')

        dataframe.loc[(
                              (dataframe['close'] < dataframe['sma_offset']) & (dataframe['rsi'] < 20)
                      ), ['buy', 'buy_tag']] = (1, 'buy_up')
    ...
    def trailing_buy_offset(self, dataframe, pair: str, current_price: float):
        ...
        if trailing_duration.total_seconds() > self.trailing_expire_seconds:
            if (current_trailing_profit_ratio > 0) and (last_candle['buy'] == 1):
                ...
                return None
        elif (('up' in str(last_candle['buy_tag']) or self.trailing_buy_uptrend_enabled) and (
    ...
    def trailing_sell_offset(self, dataframe, pair: str, current_price: float):
        ...
        if trailing_duration.total_seconds() > self.trailing_sell_expire_seconds:
            if (current_trailing_sell_profit_ratio > 0) and (last_candle['sell'] != 0):
                ...
                # wait for next signal
                return None
        elif (self.trailing_sell_downtrend_enabled and (
                trailing_duration.total_seconds() < self.trailing_sell_expire_seconds_downtrend) and (
                      current_trailing_sell_profit_ratio < (-1 * self.max_downtrend_trailing_profit))):
                        ...
                        if not trailing_sell['trailing_sell_order_started'] and (last_candle['sell'] != 0):
                            ...
                            if trailing_sell_offset == 'forcesell':
                                ...
                                self.custom_info_trail_sell[pair]['trailing_sell'][
                                    'trailing_sell_order_downlimit'] = max(current_price * (1 - trailing_sell_offset),
                                ...
                                logger.info(
                                    ...
                                    trailing_sell['start_trailing_sell_price'] * (1 - self.trailing_sell_max_sell)):
                                ...
                                logger.info(
                                    ...
                                    trailing_sell['start_trailing_sell_price'] * (1 - self.trailing_sell_max_stop)):
