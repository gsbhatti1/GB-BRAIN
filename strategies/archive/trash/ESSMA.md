...
Trends confirmation and follow through require the use of a large number of moving averages in different ways.

The concept here is to generate a combination moving average where each MA type can be weighted to provide a higher degree of trend confirmation.

The weights are configurable in settings, and as a sample 50-length has been used.

ATR did not produce good results, so it has been kept as optional.

The source code can be modified.


The indicator provides a good resistance support value on the larger time frame. It also provides breakout and breakdown indications. Follow-through is mostly effective.

The alert condition has been set up such that it can be directly ported to Discord.
