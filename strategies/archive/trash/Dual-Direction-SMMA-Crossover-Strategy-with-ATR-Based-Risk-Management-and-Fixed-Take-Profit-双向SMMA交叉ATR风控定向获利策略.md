...
plot(longTakeProfit, title="Long TP", style=plot.style_linebr, color=color.green)
plot(shortStopLoss, title="Short SL", style=plot.style_linebr, color=color.red)

```

This PineScript code defines a dual-direction trend-following strategy using SMMA crossovers and ATR for risk management. The fixed take-profit targets ensure that profits are secured once the target is reached. The dynamic stop-loss helps to protect against losses in case of adverse market movements.