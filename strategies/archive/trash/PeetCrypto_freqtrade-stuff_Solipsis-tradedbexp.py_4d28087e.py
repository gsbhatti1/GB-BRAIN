# SOURCE: https://github.com/PeetCrypto/freqtrade-stuff
# FILE  : Solipsis-tradedbexp.py
...
class Solipsis(IStrategy):
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float, current_profit: float, **kwargs) -> float:
        ...
        if self.dp and self.dp.runmode.value in ('backtest', 'hyperopt'):
            self.custom_trade_info[pair] = self.populate_trades(pair, trade=trade, time=current_time, rate=current_rate, profit=current_profit)
        ...
        min_profit = trade.calc_profit_ratio(trade.min_rate) # min array no object?
        max_profit = trade.calc_profit_ratio(trade.max_rate) # max array no object?
    ...
    def min_roi_reached(self, trade: Trade, current_profit: float, current_time: datetime) -> bool:
        ...
        if self.dp and self.dp.runmode.value in ('backtest', 'hyperopt'):
            self.custom_trade_info[trade.pair] = self.populate_trades(trade.pair, trade=trade, time=current_time, profit=current_profit)

        trade_data = self.custom_trade_info[trade.pair]
        
        open_minutes = int((current_time.timestamp() - trade.open_date_utc.timestamp()) // 60)
        min_profit = trade.calc_profit_ratio(trade.min_rate) # min array no object?
        max_profit = trade.calc_profit_ratio(trade.max_rate) # max array no object?
        profit_diff = current_profit - min_profit

        if trade.pair == 'XMR/USD':
            print(f"Trade object in ROI {trade}")
            print(f"Open Minutes: {open_minutes}")
            #print(f"Current Rate: {current_rate}")
            print(f"Current Profit: {current_profit}")
            print(f"Min Profit: {min_profit}")
            ...
            print(f"Trade Data from Populate Trades:")
            print(f"Active Trade: {trade_data['active_trade']}")
            print(f"Open Minutes: {trade_data['open_minutes']}")
            print(f"Current Profit: {trade_data['current_profit']}")
            print(f"Peak Profit DB: {trade_data['peak_profit-db']}")
            print(f"Peak Profit Ar: {trade_data['peak_profit-array']}")
            print(f"Min Profit DB: {trade_data['min_profit-db']}")
            print(f"Min Profit Ar: {trade_data['min_profit-array']}")

            print(f"Profit Array for XMR/USD:")
            print(trade_data['profit_array'])
    ...
    def populate_trades(self, pair: str, **kwargs) -> dict:
        ...
        if self.config['runmode'].value in ('live', 'dry_run'):
            ...
            if active_trade:
                ...
                trade_data['peak_profit']    = max(0, active_trade[0].calc_profit_ratio(active_trade[0].max_rate)) # if above 0
                ...
                trade_data['current_profit'] = trade_data['peak_profit'] = trade_data['min_profit'] = 0.0
        ...
        # Attempting a hack to get *some* data during backtest
        elif self.dp and self.dp.runmode.value in ('backtest', 'hyperopt'):
            # kwargs: trade=trade, time=current_time, rate=current_rate, profit=current_profit
            active_trade = kwargs.get('trade', False)

            if not 'profit_array' in self.custom_trade_info[pair]:
                self.custom_trade_info[pair]['profit_array'] = arr.array('f')
            else:
                trade_data['profit_array'] = self.custom_trade_info[pair]['profit_array']
                
            if active_trade:
                current_time = kwargs.get('time', 0)
                # current_rate = kwargs.get('rate', 0.0)
                current_profit = kwargs.get('profit', 0.0)

                open_minutes: int = (current_time - active_trade.open_date).total_seconds() // 60

                trade_data['active_trade']   = True
                trade_data['open_minutes']   = open_minutes
                trade_data['current_profit'] = current_profit
                trade_data['profit_array'].append(current_profit)

                # active_trade.adjust_min_max_rates(current_rate)

                trade_data['peak_profit-db'] = max(0, active_trade.calc_profit_ratio(active_trade.max_rate))
                trade_data['min_profit-db']  = active_trade.calc_profit_ratio(active_trade.min_rate)

                trade_data['peak_profit-array'] = max(0, max(trade_data['profit_array']))
                trade_data['min_profit-array'] = min(trade_data['profit_array'])

            else: 
                trade_data['profit_array'] = arr.array('f')
                trade_data['current_profit'] = trade_data['peak_profit'] = trade_data['min_profit'] = 0.0
                trade_data['open_minutes'] = trade_data['open_candles'] = 0
