...
The strategy uses 4 EMA lines with different periods, specifically 9-period, 26-period, 100-period, and 55-period EMA lines. The trading entry signal is to go long when the 9-period EMA line crosses the 26-period EMA line; the stop-loss exit signal is to close the position when the 100-period EMA line crosses below the 55-period EMA line. This allows fast entry and fast exit to avoid being trapped.

## Advantage Analysis

1. It is reliable to use the EMA indicator to determine trends and avoid false signals.
2. Using EMA combinations with different periods can capture short-term opportunities.
3. The fast in and fast out short-term trading method avoids long-term bearing of losses.

## Risk Analysis

1. The EMA line itself has hysteresis and may miss the best entry opportunity.
...
//strategy.entry ("sell", strategy.long, when = sell, comment = "EXIT-LONG")
