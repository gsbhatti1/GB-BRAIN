...
### Concept:
There are a large number of moving averages available. However, they are effective differently. Trends confirmation and follow-through require the use of many different moving averages.

The concept here is to generate a combined moving average where each MA type can be weighted to provide a higher degree of trend confirmation. The weights are configurable in settings, and as an example, a 50-length has been used. ATR did not produce good results, so it has been kept as optional.

The source can be modified.

### Indicator:
The indicator provides good resistance/support values on larger time frames and also indicates breakouts and breakdowns. Follow-through is mostly effective.

Alert conditions have been set up such that they can directly be ported to Discord. For alerts, users must configure their own messages.
