...
It goes long when the closing price is below the reference line, the percentage below the slow line (low offset) is greater than the configured value, the fast line is rising while the slow line is falling. When this condition satisfies, the fast and slow lines are very likely to cross soon, so it's a good entry point.

It closes long when the closing price is above the reference line, the percentage above the slow line (high offset) is greater than the configured value, the closing price has risen for 3 consecutive candles, we have open profits, and the fast line is above the slow line. If the price keeps rising after closing long, the trailing stop loss will be triggered.
...
3. Improper stop loss parameter may stop out too early or have a stop loss percentage that is too large.
...
It goes long when closing price is below reference line, percentage below slow line (low offset) is greater than configured value, fast line is rising and slow line is falling. When this condition satisfies, fast and slow lines are very likely to cross soon, so it's a good entry point.

It closes long when closing price is above reference line, percentage above slow line (high offset) is greater than configured value, closing price rose for 3 consecutive candles, we have open profits, and fast line is above slow line. If the price keeps rising after closing long, trailing stop loss will be triggered.  
