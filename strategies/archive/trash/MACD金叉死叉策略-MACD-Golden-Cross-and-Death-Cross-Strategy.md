...
```pinescript
// @version=5
// @description This strategy uses MACD DIF line with EDA line generating golden cross and death cross for entry and exit operations, backtesting shows a win rate of about 40%, BTCUSDT annualized return 1.05, also causing the number of held assets to increase continuously, thus cannot be used as an independent arbitrage strategy.

strategy("MACD Golden Cross Strategy", overlay=true)

fastLength = input(12, "Fast Line Length")
slowLength = input(26, "Slow Line Length")
MACDLength = input(9, "MACD EMA Length")

deltaIncreaseOver0 = input(color.green, 'MACD Bar Increase Above 0')
deltaIncreaseUnder0 = input(color.rgb(153, 230, 156), 'MACD Bar Increase Below 0')

deltaDecreaseOver0 = input(color.orange, 'MACD Bar Decrease Above 0')
deltaDecreaseUnder0 = input(color.red, 'MACD Bar Decrease Below 0')

buySellEnabled = input(true, 'Show Entry and Exit Signals')
...
DIF = ta.ema(close, fastLength) - ta.ema(close, slowLength) // Fast Slow EMA difference
