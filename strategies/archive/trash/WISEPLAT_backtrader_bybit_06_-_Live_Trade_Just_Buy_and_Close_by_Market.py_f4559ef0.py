...
# FILE  : 06 - Live Trade Just Buy and Close by Market.py
        ...
        for data in self.datas:  # Running through all the requested bars of all tickers
            ...
            if status in [0, 1]:
                ...
                if not self.getposition(data):  # If there is no position
                    ...
                    #     print(f"\t - Cancel the order {order.bybit_order['orderId']} to buy {data._name}")
                    ...
                    if not self.buy_once[ticker]:  # Enter long
                        ...
                        print(f"\t - The order has been submitted {self.orders[data._name].bybit_order['orderId']} to buy {data._name}")
    ...
    def notify_order(self, order):
        ...
        if order.status == bt.Order.Completed:  # If the order is fully executed
            if order.isbuy():  # The order to buy
                print(self.orders[data._name].bybit_order)  # order.executed.price, order.executed.value, order.executed.comm - you can get from here
...
if __name__ == '__main__':
    ...
    from_date = dt.datetime.now() - dt.timedelta(minutes=60)
