...
Another original idea is here with you. Actually I can say that it's a breakout strategy that uses multiple Pivot Point's averages.
...
- If both are positive or negative then the trend changes.

You have two options:
- Pivot Point Period => is the length that is used to find Pivot Points. This means it checks left/right bars if it's a Pivot Point (4 by default).
