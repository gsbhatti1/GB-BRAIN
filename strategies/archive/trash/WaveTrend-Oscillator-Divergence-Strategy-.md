...
vwapLength = input.int(200, "VWAP Length")
vwapSource = input.string("volume", title="VWAP Source")
vwap = ta.vwma(ap, vwapLength)

// Identify divergences
bullishDivergence = ta.crossover(ci, ta.ema(ci, n1))
bearishDivergence = ta.crossunder(ci, ta.ema(ci, n1))
...
stopLossLevel = input.float(2.0, title="Stop Loss Level")

stopLoss = ta.atr(atrLength) * stopLossLevel

// Position sizing
riskPercentage = input.float(2.0, "Risk Percentage (%)", minval=0.5)
positionSize = riskPercentage / 100 * account.marginBalance() - stopLoss

// Plotting indicators and signals
plot(ci, color=(ci > obLevel2 ? color.red : ci < osLevel2 ? color.green : na), title="WaveTrend Oscillator")
plot(vwap, title="VWAP", color=color.blue)
plotshape(series=bullishDivergence, location=location.belowbar, color=color.green, style=shape.triangleup, title="Bullish Divergence")
plotshape(series=bearishDivergence, location=location.abovebar, color=color.red, style=shape.triangledown, title="Bearish Divergence")

// Strategy logic
if (longCondition)
    strategy.entry("Long", strategy.long)
    strategy.exit("Take Profit", "Long", stop=ap + stopLoss)
