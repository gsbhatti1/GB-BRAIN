...
The Ichimoku Early Cloud Trend Following Strategy utilizes the Ichimoku Cloud for early trend identification, reinforced by moving average filters, to reliably detect high-quality trading opportunities. The strategy is stable with much room for enhancements and can be widely adopted for live trading.
...
lagging_span2 = donchian(lagging_span2_period + displacement)

// Check for breakout conditions
long_signal = close > lagging_span2 and close > base_line and close > conversion_line
short_signal = close < lagging_span2 and close < base_line and close < conversion_line

if (use_sl)
    stop_loss = strategy.position_avg_price * (1 - slp / 100)

// Enter trades based on signals
if (long_signal and not long_only)
...
// Manage stop loss
if (use_sl and is_long())
    strategy.exit("Exit Long", "Long", stop=stop_loss)

if (use_sl and is_short())
    strategy.exit("Exit Short", "Short", stop=stop_loss)

// Helper function to check if the position is long
is_long() =>
    strategy.position_size > 0

// Plot indicators on chart
plot(conversion_line, color=color.blue)
plot(base_line, color=color.red)
plot(lagging_span2, color=color.green)
```

This Pine Script implements the Ichimoku Cloud Strategy with the specified parameters and conditions.