...
Candlestick Size Threshold Trading Strategy-Futures-Candlestick-Size-Threshold-Trading-Strategy
...
if inTimeRange and not hasTradedToday
    var direction = na
    if high > low  // Bullish candle
        direction := long
    else  // Bearish candle
        direction := short
    
    strategy.entry("Long Trade", direction=direction, stop=low - stopLoss)
    strategy.exit("Take Profit Long", "Long Trade", limit=high + takeProfit)
