...
let pricesChart = Chart([ // This chart in JavaScript is an object, so we need to declare a configuration object variable chart before using the Chart function.
    {
        __isStock: true, // Mark whether it is a general chart. You can change it to false to run and see the result.
        tooltip: {
        ...
        xAxis: {
            type: 'datetime'
        }, // X-axis, i.e., the horizontal axis. The type is set to time.
        yAxis: { // Y-axis, i.e., the vertical axis. The scale adjusts according to the data size by default.
            title: {
                text: 'Profit'
            }, // Title
            opposite: false, // Whether to enable the right Y-axis
        },
        series: [ // Data series. This attribute stores various data series (lines, candlestick charts, labels, etc.).
            {
                name: "Zero",
                id: "Zero",
                dashStyle: 'shortdash',
...
Note: The code snippet was cut off at the end, so the `sell` function is not fully provided.