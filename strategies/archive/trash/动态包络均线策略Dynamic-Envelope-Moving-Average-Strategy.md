...
This strategy is based on moving averages and dynamic envelopes to implement both long and short trades. It tracks price breakouts beyond the envelope lines to establish positions, and closes positions when the price breaks back below the baseline moving average. This strategy works well for stocks and cryptocurrencies with obvious trends.
...
Then, it calculates the upper and lower envelope lines based on user-defined percentage parameters. For example, 5% means establishing positions when the price fluctuates 5% beyond the baseline moving average. The number of envelope lines can be customized.

Regarding entry rules, go long when the price breaks below the lower envelope line, go short when the price breaks above the upper envelope line. The rules are simple and straightforward.

Lastly, close all positions when the price breaks back below the baseline moving average. This is an exit point to trail the trend.
...
1. Automatic trend following. Using moving averages to determine trend direction is a well-established method.
2. Filtering out some noise with envelopes, preventing excessive sensitive trading. Reasonable parameter setting can greatly improve strategy profitability.
...
1. The moving average system is not sensitive to golden cross signals. It may miss some opportunities if no obvious trend exists.
2. Too wide envelope lines setting may increase trading frequency and slippage risk. Too narrow setting may miss larger moves. Finding the balance requires thorough testing.
...
5. Consider product and market differences, set multiple parameter sets suitable for different trading environments. This greatly improves strategy robustness.
...
|v_input_int_1|5|Base Moving Average Window|
