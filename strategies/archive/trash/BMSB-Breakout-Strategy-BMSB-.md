...
The core of this strategy is to use two moving averages with different periods to determine the market trend. The 20-period SMA is relatively slow, representing the medium to long-term trend of the market; the 21-period EMA is relatively fast, representing the short-term trend of the market. When the closing price crosses above the SMA, it indicates that the market has turned into an upward trend from a medium to long-term perspective, and the strategy generates a buy signal; when the closing price crosses below the EMA, it indicates that the market has turned into a downward trend from a short-term perspective, and the strategy generates a sell signal. In this way, the strategy can enter the market at an early stage of trend formation and exit in a timely manner when the trend reverses.
...
The BMSB Breakout Strategy is a simple and easy-to-use trend tracking strategy that uses two moving averages with different periods to determine the market trend. It enters the market in a timely manner when the trend is formed and exits in a timely manner when the trend reverses. The advantages of this strategy are simplicity, ease of understanding, and strong adaptability. At the same time, it also has risks such as frequent trading in oscillating markets and lagging signals. Through parameter optimization, trend filtering, risk control, and long-short timing, the performance and stability of this strategy can be further improved.
...
// Buy condition
buyCondition = close > outSma

// Sell condition
sellCondition = close < outEma
...
Note: The Pine Script code block has been completed to include the logic for entry and exit based on the defined conditions.