...
strategy.exit("Exit Long", from_entry="Long Entry", stop=TS)

// Additional code for bar color
if (Barcolor)
    plotshape(series=close > TS, location=location.abovebar, style=shape.triangleup, color=color.green, title="Profit Taken")
else
    na
```
