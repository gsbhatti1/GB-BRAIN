...
sl = sl_points * syminfo.mintick
tp = tp_points * syminfo.mintick

trailStop = strategy.position_avg_price + trail_points * syminfo.mintick

if (longSignal)
    strategy.entry("Long", strategy.long, when=longSignal)

    if (strategy.position_size > 0)
        strategy.exit("Exit Long", "Long", stop=trailStop, limit=strategy.position_avg_price + tp)

    strategy.close("Long", when=strategy.position_size <= 0 or ta.stop(strategy.position_avg_price - sl_points * syminfo.mintick))
