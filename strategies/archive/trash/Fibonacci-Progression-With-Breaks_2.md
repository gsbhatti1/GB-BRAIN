...
This indicator highlights points where price significantly deviates from a central level. This deviation distance is determined by either a user-set value or using a multiple of a period 200 ATR, and it is multiplied by successive values of the Fibonacci sequence.
...
The indicator allows highlighting potential reversal points, but it can also determine trends using the central level. An uptrend is detected when the central level is higher than its previous value; a downtrend is detected in the opposite case.

When an uptrend is detected, and the price deviates significantly upward from the central level, a checkmark will be highlighted alongside the Fibonacci sequence used as a multiplier. If the price deviates downward, a cross will be shown instead. Then, the distance threshold will be multiplied by the next value in the Fibonacci sequence.
...
Upper and lower extremities made from the central level and threshold distance are highlighted and can be used as support and resistance levels.

## Backtest

![Backtest Image](https://www.fmz.com/upload/asset/c3049b5140ba230397.jpg)
