...
def calcRecords(records, period=30, start=None):
    ...
    period_ms = period * 60000 # K-line period in milliseconds
    end_in = records[len(records)-1]['Time'] # The end time of the K line
    start_at = records[0]['Time'] # The start time of the K line

    # Get the starting time that can be used to calculate the target K-line
    r_offest = start_at % period_ms
    start_at = start_at - r_offest + period_ms
    if start is not None:
        start_at = start

    target_count = ceil((end_in-start_at)/period_ms) #The number of target K lines
...
def main():
    ...
    M15_GEN = ext.calcRecords(M1, 15) # Use the ext.calcRecords function to calculate the 15-minute K-line based on the 1-minute K-line
    Log(M15[len(M15)-50:])
