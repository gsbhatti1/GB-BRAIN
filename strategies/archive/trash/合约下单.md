...
|----|----|----|
|SLIP_POINT|0.01|Slippage|
|TRADE_TIMEOUT|10|Transaction timeout|
|FACE_VALUE|10|FACE VALUE|
|PRICE_PRECISION|4|Price precision|
|AMOUNT_PRECISION|8|Trading volume precision|
...
if (!order_id) return Log('Incorrect order id', order_id);
...
Log('Transaction timeout, order cancelled' + oid + COLOR.RED);
