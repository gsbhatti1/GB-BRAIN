...
//Entry--
//Calculate how many units can be purchased based on risk management and stop loss
qty1 = (strategy.equity * riskCapital / 100) / (close * stopLoss / 100)

//Check if cash is sufficient to buy qty1, if capital not available use the available capital only
if (strategy.equity < close * qty1 * stopLoss / 100)
    qty1 := strategy.equity / (close * stopLoss / 100)

//Exit Logic
if (rsiOfMACD >= 80) and takeProfit == true
    strategy.close("Take Profit", comment="Close position at profit")
    
if (rsiOfMACD <= 15)
    strategy.close(comment="Stop Loss hit")

```

This script defines a trading strategy based on the RSI of MACD indicators. It calculates the quantity to trade and sets conditions for entry and exit points, including stop loss and take profit.