...
```pinescript
//@version=5
strategy(title="VWAP with SuperTrend Buy/Sell Strategy", shorttitle="VWAPST", overlay=true)
...
// Ensure the signal is only triggered when there is a change in direction.
if (buysignal)
