...
"Trading the equity curve" as a risk management method is the process of acting on trade signals depending on whether a system’s performance is indicating that the strategy is in a profitable or losing phase.

The point of managing the equity curve is to minimize risk in trading when the equity curve is in a downtrend. This strategy has two modes to determine the equity curve downtrend: By creating two simple moving averages (SMA) of a portfolio's equity curve—a short-term and a longer-term one—and acting on their crossings. If the fast SMA is below the slow SMA, an equity downtrend is detected (smafastequity < smaslowequity). The second method is by using the crossings of equity itself with the longer-period SMA (equity < smasloweequity).
...
| v_input_string_1 | 0 | Position Size Adjustment: Reduce size by (%) or Increase size by (%) |
...
//is indicating that the strategy is in a profitable or losing phase.
//The point of managing the equity curve is to minimize risk in trading when the equity curve is  in a downtrend. 
//This strategy has two modes to determine the equity curve downtrend:
//By creating two simple moving averages (SMA) of a portfolio's equity curve—a short-term
...
strategy("Use Trading the Equity Curve Position Sizing", shorttitle="TEC", default_qty_type = strategy.percent_of_equity, default_qty_value = 10, initial_capital = 100000)
