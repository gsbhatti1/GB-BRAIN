...
1. Calculate the fast DEMA line (8-day), slow DEMA line (24-day), and auxiliary DEMA line (configurable).
...
3. Add a signal filter that signals are only triggered when the current value of the auxiliary line is higher than the previous day, avoiding false breakouts.

4. Adopt a trend following stop loss mechanism where the stop loss line adjusts based on price movement, locking in partial profits.
...
2. Double DEMA algorithm is smoother, avoids overfitting, and provides more reliable signals.
...
1. Frequent trading could occur in ranging markets, increasing exposure and causing losses.

2. Overly large fixed stop loss percentage may trigger unwanted big stop losses in extreme price swings.

3. DEMA crossover signals lag, and long entries at peaks may increase loss risks in fast-moving markets.

4. In live trading, slippage affects profitability, and parameter tuning is needed.
